...
def add_resume():
    ...
    if request.method=='POST':
        ...
        extra_act=request.form['extra_act']
        ...
        mycursor.execute("update vh_candidate set interests=%s,extra_act=%s,sports=%s where username=%s",(interests,extra_act,sports,uname))
        mydb.commit()
...
def userhome():
    ...
    if mot=="1":
        mst="1"
        mm=""
        mycursor.execute("SELECT * FROM vh_motivation order by rand() limit 0,1")
        mdt = mycursor.fetchall()
        for mdt1 in mdt:
            mm=mdt1[1]
        mess=mm
    ...
    return render_template('userhome.html',data=data,act=act,pid=pid,data1=data1,msg=msg,st=st,mobile=mobile,name=name,mess=mess,mst=mst)
